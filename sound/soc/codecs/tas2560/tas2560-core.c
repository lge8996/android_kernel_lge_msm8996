...
int tas2560_parse_dt(struct device *dev, struct tas2560_priv *pTAS2560)
{
	...
	pTAS2560->mnResetGPIO1 = of_get_named_gpio(np, "ti,reset-gpio1", 0);
	if (!gpio_is_valid(pTAS2560->mnResetGPIO1)) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
			"ti,reset-gpio1", np->full_name, pTAS2560->mnResetGPIO1);
	} else {
		dev_dbg(pTAS2560->dev, "ti,reset-gpio1=%d", pTAS2560->mnResetGPIO1);
	}
	pTAS2560->mnResetGPIO2 = of_get_named_gpio(np, "ti,reset-gpio2", 0);
	if (!gpio_is_valid(pTAS2560->mnResetGPIO2)) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
			"ti,reset-gpio2", np->full_name, pTAS2560->mnResetGPIO2);
	} else {
		dev_dbg(pTAS2560->dev, "ti,reset-gpio2=%d", pTAS2560->mnResetGPIO2);
	}
