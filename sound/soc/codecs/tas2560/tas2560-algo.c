...
static int smartamp_bypass = FALSE;
static int smartamp_enable = FALSE;
static int calib_state = 0;
static uint32_t calibration_data[3] = {0,0,0};
...
static int get_calibrated_re_tcalib (uint32_t *rdc_fix, uint32_t *tv_fix, int channel_count)
{
	...
	file = filp_open(filepath, O_RDONLY, 0666);
	...
	if(!IS_ERR(file))
	{
		...
		if (channel_count == 1) {
			if(sscanf(calib_data, "%u;%u;", rdc_fix, tv_fix) != 2) {
				pr_err("[Smartamp:%s] file %s read error\n", __func__, filepath);
				ret = -EIO;
			}
		} else if (channel_count == 2) {
			if(sscanf(calib_data, "%u;%u;%u;", &rdc_fix[0], &rdc_fix[1], tv_fix) != 3) {
				pr_err("[Smartamp:%s] file %s read error\n", __func__, filepath);
				ret = -EIO;
			}
		}
...
/*Control: Set Profile*/
static const char *profile_index_text[] = {"NONE","MUSIC","VOICE","VOIP", "RING", "CALIB"};
		...
		struct snd_ctl_elem_value *pUcontrol)
{
	...
	int param_id = 0;
	int total_profile_count = sizeof(profile_index_text)/sizeof(profile_index_text[0]);

	if (user_data >= total_profile_count)
		return -EINVAL;
...
/*Control: Calibration and Test(F0,Q,Tv) Controls*/
...
static int tas2560_calib_set(struct snd_kcontrol *pKcontrol,
		struct snd_ctl_elem_value *pUcontrol)
{
	int ret = 0;
	int param_id = 0;
	...
	if (user_data < 0 || user_data >= ARRAY_SIZE(tas2560_calib_test_text)) {
		pr_err("%s: [Smartamp] user_data %d is not valid\n", __func__, user_data);
		return -EINVAL;
	}
	...
	switch(user_data) {
		case CALIB_START:
			...
			param_id = TAS_CALC_PARAM_IDX(TAS_SA_CALIB_INIT, 1, CHANNEL1);
			ret = afe_smartamp_algo_ctrl((u8 *)&data, param_id,
					TAS_SET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
			if(ret < 0) {
				pr_err("TI-SmartPA: %s: Failed to set calib/test", __func__);
		...
		case CALIB_STOP:
			...
			ret = afe_smartamp_algo_ctrl((u8 *)&data, param_id,
					TAS_SET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
			if(ret < 0) {
				pr_err("TI-SmartPA: %s: Failed to set calib deinit", __func__);
				break;
			}

			param_id = TAS_CALC_PARAM_IDX(TAS_SA_CALIB_DEINIT, 1, CHANNEL1);
			ret = afe_smartamp_algo_ctrl((u8 *)&data, param_id,
					TAS_SET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
			if(ret < 0) {
				pr_err("TI-SmartPA: %s: Failed to set calib deinit", __func__);
		...
		case TEST_STOP:
		default:
			pr_info("TI-SmartPA: %s: Unsupported", __func__);
			break;
	}
...
static int tas2560_calib_get(struct snd_kcontrol *pKcontrol,
		struct snd_ctl_elem_value *pUcontrol)
{
	int ret = 0;
...
static int tas2560_get_re_right(struct snd_kcontrol *pKcontrol,
		struct snd_ctl_elem_value *pUcontrol)
{
	int ret;
	int user_data = 0;
	...
	if(TRUE == smartamp_enable)
	{
		param_id = TAS_CALC_PARAM_IDX(TAS_SA_GET_RE, 1, CHANNEL1);
		ret = afe_smartamp_algo_ctrl((u8 *)&user_data, param_id,
				TAS_GET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
		if(ret < 0)
		{
...
static int tas2560_get_f0_right(struct snd_kcontrol *pKcontrol,
		struct snd_ctl_elem_value *pUcontrol)
{
	int ret;
	int user_data = 0;
	...
	if(TRUE == smartamp_enable)
	{
		param_id = TAS_CALC_PARAM_IDX(TAS_SA_GET_F0, 1, CHANNEL1);
		ret = afe_smartamp_algo_ctrl((u8 *)&user_data, param_id,
				TAS_GET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
		if(ret < 0)
		{
...
static int tas2560_get_q_right (struct snd_kcontrol *pKcontrol,
		struct snd_ctl_elem_value *pUcontrol)
{
	int ret;
	int user_data = 0;
	...
	if(TRUE == smartamp_enable)
	{
		param_id = TAS_CALC_PARAM_IDX(TAS_SA_GET_Q, 1, CHANNEL1);
		ret = afe_smartamp_algo_ctrl((u8 *)&user_data, param_id,
				TAS_GET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
		if(ret < 0)
		{
...
static int tas2560_get_tv_right(struct snd_kcontrol *pKcontrol,
		struct snd_ctl_elem_value *pUcontrol)
{
	int ret;
	int user_data = 0;
	...
	if(TRUE == smartamp_enable)
	{
		param_id = TAS_CALC_PARAM_IDX(TAS_SA_GET_TV, 1, CHANNEL1);
		ret = afe_smartamp_algo_ctrl((u8 *)&user_data, param_id,
				TAS_GET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
		if(ret < 0)
		{
		...
		struct snd_ctl_elem_value *pUcontrol)
{
	...
	int param_id = 0;
	int user_data = pUcontrol->value.integer.value[0];
	int smartamp_enable_req = user_data;

	if (user_data < 0 || user_data >= ARRAY_SIZE(tas2560_smartamp_enable_text)) {
		pr_err("%s: [Smartamp] user_data %d is not valid\n", __func__, user_data);
		return -EINVAL;
	}
	...
	if (FALSE == smartamp_enable_req) {
		...
		if(ret) pr_err("TI-SmartPA: %s: FB Path Info failed ignoring ret = 0x%x", __func__, ret);
		...
		if(ret < 0) pr_err("TI-SmartPA: %s: Failed to set config", __func__);

		/* during calibration, the following may fail */
		smartamp_enable = TRUE;

		ret = get_calibrated_re_tcalib (calibration_data, &calibration_data[2], 2);
		if(ret) {
			...
			ret = afe_smartamp_algo_ctrl ((u8 *)(&(calibration_data[0])),
			...
			ret = afe_smartamp_algo_ctrl ((u8 *)&(calibration_data[1]),
					param_id, TAS_SET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
			if(ret < 0) {
				pr_err("TI-SmartPA: %s: Failed to set Re", __func__);
				ret = -EINVAL;
			...
			ret = afe_smartamp_algo_ctrl ((u8 *)&(calibration_data[2]),
					param_id, TAS_SET_PARAM, sizeof(uint32_t), AFE_SMARTAMP_MODULE_RX);
			if(ret < 0) {
...
{
	int ret = 0;
	int user_data = smartamp_enable;
	pUcontrol->value.integer.value[0] = user_data;
	pr_info("TI-SmartPA: %s: case %s", __func__, tas2560_smartamp_enable_text[user_data]);
	return ret;
}

/*Control: Smartamp Bypass */
		...
		struct snd_ctl_elem_value *pUcontrol)
{
	...
	int user_data = pUcontrol->value.integer.value[0];
        ...
        if (user_data < 0 || user_data >= ARRAY_SIZE(tas2560_smartamp_bypass_text)) {
                pr_err("%s: [Smartamp] user_data %d is not valid\n", __func__, user_data);
                return -EINVAL;
        }
...
static const struct snd_kcontrol_new smartamp_tas2560_mixer_controls[] = {
	SOC_SINGLE_EXT("TAS2560_SMARTPA_ENABLE", SND_SOC_NOPM, 0, 1, 0,
	...
	SOC_ENUM_EXT("TAS2560_ALGO_CALIB_TEST", tas2560_calib_test_enum[0],
			tas2560_calib_get, tas2560_calib_set),

	/* Get Re Tv Q F0 */
	SOC_SINGLE_EXT("TAS2560_GET_F0_LEFT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_f0_left, NULL),
	SOC_SINGLE_EXT("TAS2560_GET_F0_RIGHT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_f0_right, NULL),

	SOC_SINGLE_EXT("TAS2560_GET_Q_LEFT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_q_left, NULL),
	SOC_SINGLE_EXT("TAS2560_GET_Q_RIGHT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_q_right, NULL),

	SOC_SINGLE_EXT("TAS2560_GET_TV_LEFT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_tv_left, NULL),
	SOC_SINGLE_EXT("TAS2560_GET_TV_RIGHT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_tv_right, NULL),

	SOC_SINGLE_EXT("TAS2560_GET_RE_LEFT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_re_left, NULL),
	SOC_SINGLE_EXT("TAS2560_GET_RE_RIGHT", SND_SOC_NOPM, 0, 0x7fffffff, 0,
			tas2560_get_re_right, NULL),
};
