...
struct rt1711_chip {
    ...
    struct gpio_desc *vconn_en_gpio;       //PM8996  GPIO 1
    struct gpio_desc *redriver_sel_gpio;   //MSM8996 GPIO 96
    struct gpio_desc *usb_ss_en_gpio;      //        GPIO 86
    struct gpio_desc *sbu_en_gpio;         //        GPIO 90
    struct gpio_desc *sbu_sel_gpio;        //PM8996  GPIO 3
#endif
};

static struct rt1711_chip _rt1711_dev;
...
static int rt1711_init_alert(struct tcpc_device *tcpc)
{
	...
	char *name;
	int len;
	chip->rt1711_pinctrl = devm_pinctrl_get(&(chip->client->dev));
	/* Clear Alert Mask & Status */
	rt1711_write_word(chip->client, TCPC_V10_REG_ALERT_MASK, 0);
	rt1711_write_word(chip->client, TCPC_V10_REG_ALERT, 0xffff);
	 
	chip->rt1711_pinctrl_state_active = pinctrl_lookup_state(chip->rt1711_pinctrl, "rt1711_int_active");

        if (IS_ERR(chip->rt1711_pinctrl_state_active)){

                printk("%s: failed at rt1711 pinctrl_lookup_state\n", __func__);

                return PTR_ERR(chip->rt1711_pinctrl_state_active);
...
static int rt1711_get_cc(struct tcpc_device *tcpc, int *cc1, int *cc2)
{
	...
	*cc2 = TCPC_V10_REG_CC_STATUS_CC2(status);
...
#ifdef CONFIG_LGE_USB
	if ((*cc1 + *cc2) != TYPEC_CC_VOLT_OPEN &&
	    (*cc1 == TYPEC_CC_VOLT_OPEN || *cc2 == TYPEC_CC_VOLT_OPEN)) {
		power_ctrl = rt1711_i2c_read8(tcpc, TCPC_V10_REG_POWER_CTRL);
		if (power_ctrl < 0)
			return power_ctrl;

		if (power_ctrl & TCPC_V10_REG_POWER_CTRL_VCONN) {
	...
	if ((*cc1 + *cc2) == TYPEC_CC_VOLT_OPEN &&
		...
		pr_info("RT1711H: CC Removal\n");
		gpiod_direction_output(_rt1711_dev.usb_ss_en_gpio, 0);
		udelay(200);
		gpiod_direction_output(_rt1711_dev.redriver_sel_gpio, 0);
	}
#endif
...
#ifdef CONFIG_LGE_USB
	pr_info("RT1711H: Orientation: CC%d\n", (polarity + 1));
	gpiod_direction_output(_rt1711_dev.usb_ss_en_gpio, 1);
	udelay(200);
	gpiod_direction_output(_rt1711_dev.redriver_sel_gpio, polarity ? 0 : 1);
#endif
...
#ifdef CONFIG_LGE_USB
	if (!IS_ERR_OR_NULL(_rt1711_dev.vconn_en_gpio))
		gpiod_direction_output(_rt1711_dev.vconn_en_gpio, enable);
#endif
...
#ifdef CONFIG_LGE_USB
	_rt1711_dev.vconn_en_gpio = devm_gpiod_get(dev, "rt1711,vconn-en",
			GPIOD_OUT_LOW);
	if(IS_ERR(_rt1711_dev.vconn_en_gpio)) {
		dev_err(dev, "failed to allocate vconn_en gpio\n");
		_rt1711_dev.vconn_en_gpio = NULL;
	}

	//Enable REDRIVER_SEL
	_rt1711_dev.redriver_sel_gpio = devm_gpiod_get(dev, "rt1711,redriver-sel",
			GPIOD_OUT_LOW);
	if(IS_ERR(_rt1711_dev.redriver_sel_gpio)) {
		dev_err(dev, "failed to allocate redriver_sel gpio\n");
		_rt1711_dev.redriver_sel_gpio = NULL;
	}

	//Enable USB_SS_EN
	_rt1711_dev.usb_ss_en_gpio = devm_gpiod_get(dev, "rt1711,usb-ss-en",
			GPIOD_OUT_HIGH);
	if(IS_ERR(_rt1711_dev.usb_ss_en_gpio)) {
		dev_err(dev, "failed to allocate usb_ss_en  gpio\n");
		_rt1711_dev.usb_ss_en_gpio = NULL;
	}

	pr_info("DEBUG: usb_ss_en_gpio high\n");
	gpiod_direction_output(_rt1711_dev.usb_ss_en_gpio,1);

	_rt1711_dev.sbu_sel_gpio = devm_gpiod_get(dev, "rt1711,sbu-sel",
			GPIOD_OUT_HIGH);
	if(IS_ERR(_rt1711_dev.sbu_sel_gpio)) {
		dev_err(dev, "failed to allocate sbu_sel gpio\n");
		_rt1711_dev.sbu_sel_gpio = NULL;
	}


	_rt1711_dev.sbu_en_gpio = devm_gpiod_get(dev, "rt1711,sbu-en",
			GPIOD_OUT_HIGH);
	if(IS_ERR(_rt1711_dev.sbu_en_gpio)) {
		dev_err(dev, "failed to allocate sbu_en gpio\n");
		_rt1711_dev.sbu_en_gpio = NULL;
	}
#endif
...
#ifdef CONFIG_LGE_USB
void handle_sbu_switch(bool enable)
{
	if(enable) {
		/* SBU_EN low, SBU_SEL high */
		if(_rt1711_dev.sbu_en_gpio) {
            pr_info("RT1711H: /OE - SBU_EN (0) - Turn On\n");
			gpiod_direction_output(_rt1711_dev.sbu_en_gpio,0);
			usleep_range(40000, 41000); /* Turn-On Time /OE */
		}
		if(_rt1711_dev.sbu_sel_gpio) {
            pr_info("RT1711H: SEL - SBU_SEL (1) - SBU Path : USB_ID\n");
			gpiod_direction_output(_rt1711_dev.sbu_sel_gpio, 1);
			usleep_range(40000, 41000);
		}
	} else {
		/* SBU_EN high */
		if(_rt1711_dev.sbu_en_gpio) {
            pr_info("RT1711H: /OE - SBU_EN (0) - Turn Off\n");
			gpiod_direction_output(_rt1711_dev.sbu_en_gpio,1);
