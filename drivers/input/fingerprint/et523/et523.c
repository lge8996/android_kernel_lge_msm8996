...
struct wake_lock et523_wake_lock;
...
static irqreturn_t fp_eint_func(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&et523_wake_lock, msecs_to_jiffies(1500));
	return IRQ_HANDLED;
}
...
static int etspi_remove(struct platform_device *pdev)
{
	...
	//del_timer_sync(&fps_ints.timer);
	wake_lock_destroy(&et523_wake_lock);
	request_irq_done = 0;
	/* t_mode = 255; */
	return 0;
}
...
*/
	...
	wake_lock_init(&et523_wake_lock, WAKE_LOCK_SUSPEND, "et523_wake_lock");
