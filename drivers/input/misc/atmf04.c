...
static int controller_regulator_configure(struct atmf04_data *data, bool on)
{
  ...
  if (on == false)
    goto hw_shutdown;
/*
  pdata->vcc_i2c = regulator_get(&client->dev, "Adsemicon,vdd_i2c");
  ...
  if (regulator_count_voltages(pdata->vcc_i2c) > 0) {
    rc = regulator_set_voltage(pdata->vcc_i2c, pdata->vdd_ana_supply_min,
...
/*  if (regulator_count_voltages(pdata->vcc_i2c) > 0)
    regulator_set_voltage(pdata->vcc_i2c, 0, pdata->vdd_ana_supply_max);
*/
  regulator_put(pdata->vcc_ana);
...
/*
  rc = regulator_set_load(pdata->vcc_i2c, pdata->vdd_ana_load_ua);
  if (rc < 0) {
    dev_err(&client->dev,
        "Regulator vcc_ana set_opt failed rc=%d\n", rc);
    return rc;
