...
extern int ft5726_reg_read(struct device *dev, u16 addr, void *data, int size);
extern int ft5726_reg_write(struct device *dev, u16 addr, void *data, int size);
...
static int fts_rw_iic_drv_RDWR(struct i2c_client *client, unsigned long arg)
{
	...
	i2c_rw_msg = (struct fts_rw_i2c *)kmalloc(i2c_rw_queue.queuenum * sizeof(struct fts_rw_i2c),
				GFP_KERNEL);
	if (!i2c_rw_msg)
		return -ENOMEM;
