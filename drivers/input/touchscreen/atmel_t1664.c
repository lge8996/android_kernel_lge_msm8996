...
//static int get_batt_temp(void); //bring up
...
static int mxt_update_file_name(struct device *dev, char **file_name, const char *buf, size_t count)
{
	...
	/* Simple sanity check */
	if (count > 128) {
		TOUCH_ERR_MSG("File name too long %zd\n", count);
		return -EINVAL;
	}
...
static ssize_t mxt_update_patch_store(struct mxt_data *data, const char *buf, size_t count)
{
	...
	TOUCH_DEBUG_MSG("%s patch file size:%zd\n", __func__, fw->size);
...
static irqreturn_t touch_irq_handler(int irq, void *dev_id)
{
	...
	if (atomic_read(&data->state.pm) >= PM_SUSPEND) {
		TOUCH_INFO_MSG("interrupt in suspend[%d]\n", atomic_read(&data->state.pm));
		atomic_set(&data->state.pm, PM_SUSPEND_IRQ);
...
static void mxt_regulator_enable(struct mxt_data *data)
{
	...
	TOUCH_DEBUG_MSG("%s() reset:%d, avdd:%d, dvdd:%d, vio:%d\n",__func__,
			...
			gpio_get_value(data->pdata->gpio_avdd),
			gpio_get_value(data->pdata->gpio_dvdd),
			gpio_get_value(data->pdata->gpio_vio));
}
	...
	if (le32_to_cpu(fw_img->magic_code) != MXT_FW_MAGIC) {
		...
		if (fw_info->hdr_len != sizeof(struct mxt_fw_image)
			...
			if (ppheader->magic == MXT_PATCH_MAGIC) {
				...
				TOUCH_ERR_MSG("Firmware file is invaild !!hdr size[%d] cfg,fw size[%d,%d] filesize[%zd]\n",
					fw_info->hdr_len, fw_info->cfg_len, fw_info->fw_len, fw->size);
				return -EINVAL;
			}
		}
...
static int mxt_suspend(struct device *dev)
{
	...
	if(!is_probing) {
		...
		trigger_touch_temp(24); //bring up
		atomic_set(&data->state.pm, PM_SUSPEND);
	}
	return 0;
}
...
static int mxt_resume(struct device *dev)
{
	...
	if(!is_probing) {
		...
		if(atomic_read(&data->state.pm) == PM_SUSPEND_IRQ) {
			...
			atomic_set(&data->state.pm, PM_RESUME);
...
static int __init mxt_init(void)
{
	...
	TOUCH_INFO_MSG("rev_str[%d] : %s\n", board_rev, rev_str[board_rev]);

	if (strcmp(rev_str[board_rev], "rev_10")) { // if device is not rev_10
		TOUCH_INFO_MSG("Touch IC is not atmel.\n");
		return 0;
	}

	TOUCH_INFO_MSG("Atmel init!\n");
...
static void __exit mxt_exit(void)
{
	...
	if (strcmp(rev_str[board_rev], "rev_10")) { // if device is not rev_10
		TOUCH_INFO_MSG("mxt_exit, not Atmel.\n");
		return;
	}
