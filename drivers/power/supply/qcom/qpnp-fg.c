...
#ifdef CONFIG_MACH_MSM8996_TF10
		if(chip->dummy_battery_fake_temp)
			val->intval = 250;
#endif
		break;
...
wait:
	...
	if (of_property_read_bool(profile_node,
				"lge,dummy_battery_fake_temp"))
		chip->dummy_battery_fake_temp = true;
	else
		chip->dummy_battery_fake_temp = false;

	pr_err("lge_get_board_revno = %d \n", lge_get_board_revno());
