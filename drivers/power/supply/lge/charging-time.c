...
void estimate_bsm_curr(int bsm_watt){
	double est_bsm_curr;

	if(time_me.fcc_votable == NULL)
		time_me.fcc_votable = find_votable("FCC");

	if(bsm_watt == -1){
		if(time_me.fcc_votable != NULL){
			vote(time_me.fcc_votable, "BSM_TTF_VOTER", false, 0);
			pr_info("[BSM]vote release\n");
		}
		return;
	}
	est_bsm_curr = (bsm_watt / 4400) * 1000;
	pr_info("[BSM] Current = %d\n",(int)est_bsm_curr);
	if((int)est_bsm_curr > 500)
		vote(time_me.fcc_votable, "BSM_TTF_VOTER", true, ((int)est_bsm_curr)*1000);
	return;
}

int estimate_bsm_watt(int bsm_ttf, int raw_soc, int power_now) {
	int remain_watt = 0;
	int remain_time;
	double bsm_watt;

	remain_time = charging_time_remains(raw_soc);

	remain_watt = (remain_time * (power_now/1000)) / 3600;
	bsm_watt = ((double)remain_watt / (remain_time + bsm_ttf)) * 3600;
	pr_info("[BSM]RemainWatt=%d, EstWatt=%d, RemainTime=%d, EstTime=%d, POWER_NOW=%d\n",
			remain_watt, (int)bsm_watt, remain_time,(remain_time + bsm_ttf) ,power_now / 1000);
	return (int)bsm_watt;
}
static void chgtime_dworkf(struct work_struct *work) {
	...
	int bsm_ttf , raw_soc, power_now;
	struct power_supply* psy_veneer = power_supply_get_by_name("veneer");
	struct power_supply* psy_battery = power_supply_get_by_name("battery");
	struct power_supply* psy_usb = power_supply_get_by_name("usb");

	power_supply_get_property(psy_veneer, POWER_SUPPLY_PROP_BSM_TTF, &val);
	bsm_ttf = val.intval;

	power_supply_get_property(psy_battery, POWER_SUPPLY_PROP_CAPACITY_RAW, &val);
	raw_soc = val.intval;

	power_supply_get_property(psy_usb, POWER_SUPPLY_PROP_POWER_NOW, &val);
	power_now = val.intval;
	...
	pr_err("[BSM]Watt before = %d bsm_ttf = %d \n", time_me.profile_power, bsm_ttf);
	if (time_me.bsm_ttf_estimate == 1 && bsm_ttf){
		time_me.bsm_ttf_estimate = estimate_bsm_watt(bsm_ttf, raw_soc, power_now);
		estimate_bsm_curr(time_me.bsm_ttf_estimate);
		time_me.profile_power = time_me.bsm_ttf_estimate;
		pr_err("[BSM]Watt %d \n", time_me.profile_power);
	}
	chgtime_profiling(time_me.profile_power, time_me.chgtime_fullraw);
	time_me.chgtime_notify(time_me.profile_power);

	pr_err("[BSM]Time to full now =  %d\n",charging_time_remains(raw_soc));
	pr_err("[BSM]Watt before = %d bsm_ttf = %d \n", time_me.profile_power, bsm_ttf);
	if(!time_me.bsm_ttf_estimate && bsm_ttf){
		time_me.bsm_ttf_estimate = 1;
		schedule_delayed_work(&time_me.chgtime_dwork, 0);
	}
}
...
void charging_time_clear(void) {
	...
	time_me.bsm_ttf_estimate = 0;
	if(time_me.fcc_votable != NULL){
		vote(time_me.fcc_votable, "BSM_TTF_VOTER", false, 0);
		pr_err("[BSM} vote release in clear");
	}
